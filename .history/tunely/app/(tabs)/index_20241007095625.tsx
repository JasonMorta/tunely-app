...
import { ref, onValue, off } from "firebase/database"; // Firebase database methods
...
const Index: React.FC = () => {
  ...
  useEffect(() => {
    const dbRef = ref(database, 'events'); // Reference to your events in Firebase
    ...
    const unsubscribe = onValue(dbRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setEvents(formattedEvents); // Update state with the new list of events
      } else {
        console.log("No events data available");
        setEvents([]); // Clear the events list if no data is available
      }
