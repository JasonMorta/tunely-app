...
import placeholderImage from '../../assets/images/temp.jpg'; // Import a placeholder image
...
const Index: React.FC = () => {
  ...
  const renderItem = ({ item }: { item: any }) => {
    // Log the item.image value
    console.log(`Event ID: ${item.id}, Image URL: ${item.image}`);

    const isImageError = imageErrorIds.includes(item.id);

    return (
      <Pressable style={styles.eventItem} onPress={() => openModal(item)}>
        <Image
          source={item.image && !isImageError && item.image.trim() !== '' ? { uri: item.image } : placeholderImage} // Conditional source
          style={styles.eventImage}
          resizeMode="cover" // Adjust as needed
          onError={() => handleImageError(item.id)} // Handle image load error
        />
